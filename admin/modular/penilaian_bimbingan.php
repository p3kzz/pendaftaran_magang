...
if (isset($_POST['nilai'])) {
    $nilai_bimbingan = trim($_POST['nilai']);
    $laporan_id = $laporan['laporan_id'];
    $mahasiswa_id = $laporan['mahasiswa_id'];
    $pembimbing_id = $laporan['pembimbing_id'];
    $checkNilai = $conn->prepare("SELECT id FROM penilaian WHERE laporan_id = ? AND pembimbing_id = ?");
    $checkNilai->bind_param("ii", $laporan_id, $pembimbing_id);
    $checkNilai->execute();
    ...
    if (!empty($nilai_bimbingan)) {
        $conn->begin_transaction();

        try {
            $insertNilai = $conn->prepare("INSERT INTO penilaian (nilai_bimbingan, laporan_id, mahasiswa_id, pembimbing_id, created_at) VALUES (?, ?, ?, ?, NOW())");
            $insertNilai->bind_param("diii", $nilai_bimbingan, $laporan_id, $mahasiswa_id, $pembimbing_id);
            $insertNilai->execute();
